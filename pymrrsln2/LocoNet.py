...
class LocoNet:
  ...
  @param port_cfg Dictionary with the port settings. It has to have thesse keys:
                    'device'    : device name (e.g. /dev/ttyACM0)
  ...
  def __init__(self, port_cfg):
    ...
    # lock to guard access into `msg_queue`
    self.msg_lock = threading.Lock()
    # queue of messages
    self.msg_queue = Queue.Queue()

    if port_cfg['device'] == None:
      self.com = None
    else:
      if 'baud_rate' in port_cfg:
        try:
          # 8 bits, 1 stop bit, 1 start bit
          self.com = serial.Serial(
            port = port_cfg['device'],
            baudrate = port_cfg['baud_rate'],
        ...
        print("ERROR: 'baud_rate' was not specified in the config file.", file=sys.stderr)
        exit(-1)
